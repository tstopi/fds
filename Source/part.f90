...
REAL(EB) :: UBAR,VBAR,WBAR,RVC,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_AIR, &
            ...
            GX_LOC,GY_LOC,GZ_LOC,DUMMY,DRAG_MAX(3),TAU_P,TAU_T,K_SGS,DELTA
...
ELSE PARTICLE_NON_STATIC_IF ! Drag calculation for stationary, airborne particles
   ...
   SELECT CASE (LPC%DRAG_LAW)
      ...
      CASE(USER_DRAG)
         Y_TERM = LP%PWT*RVC*(0.5_EB*LPC%DRAG_COEFFICIENT*A_DRAG*QREL + LP%M_DOT/RHO_G)
         LP%ACCEL_X = -UBAR*Y_TERM(1)
         LP%ACCEL_Y = -VBAR*Y_TERM(2)
         LP%ACCEL_Z = -WBAR*Y_TERM(3)
      CASE (SCREEN_DRAG)
     ...
     CASE(BUNDLE_DRAG)
         ...
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
         CALL GET_VISCOSITY(ZZ_GET,MU_AIR,TMP_G)
         QREL = SQRT(UBAR**2+VBAR**2+WBAR**2)
         QREL = QREL/LPC%FREE_AREA_FRACTION
         LP%RE  = RHO_G*QREL*2._EB*R_D/MU_AIR
         C_DRAG = DRAG(LP%RE,CYLINDER_DRAG)

   END SELECT
   DRAG_MAX(1) = (LP%ACCEL_X- RDXN(IIG)*(HS(IIG+1,JJG,KKG)-HS(IIG,JJG,KKG)))*DT_P/(-UBAR+TWO_EPSILON_EB)
   DRAG_MAX(2) = (LP%ACCEL_Y- RDYN(JJG)*(HS(IIG,JJG+1,KKG)-HS(IIG,JJG,KKG)))*DT_P/(-VBAR+TWO_EPSILON_EB)
   DRAG_MAX(3) = (LP%ACCEL_Z- RDZN(KKG)*(HS(IIG,JJG,KKG+1)-HS(IIG,JJG,KKG)))*DT_P/(-WBAR+TWO_EPSILON_EB)
