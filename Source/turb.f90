...
REAL(EB) :: TEMP_TERM,DUDY,DUDZ,DVDX,DVDZ,DWDX,DWDY,ONTHDIV,CLIM,DELTA
INTEGER :: I,J,K
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: L11,L22,L33,L12,L13,L23,ML,MLHAT,KSGS,UPHAT,VPHAT,WPHAT
...
UPHAT=> WORK9; CALL TEST_FILTER(UPHAT,UP)
KSGS = 0.5*(UP-UPHAT)**2
VPHAT=> WORK9; CALL TEST_FILTER(VPHAT,VP)
KSGS = KSGS+ 0.5*(VP-UPHAT)**2
WPHAT=> WORK9; CALL TEST_FILTER(WPHAT,WP)
KSGS = KSGS+ 0.5*(WP-WPHAT)**2
...
DO K = 1,KBAR
   DO J = 1,JBAR
      DO I = 1,IBAR
         ...
         CLIM = 24._EB/(23._EB*SQRT(3._EB))*KSGS(I,J,K)/(DELTA**2*STRAIN_RATE(I,J,K)**2)
         IF (CSD2(I,J,K) > CLIM) CSD2(I,J,K)=CLIM
         IF (CSD2(I,J,K) < -CLIM) CSD2(I,J,K)=-CLIM
      END DO
   END DO
END DO
